...
public:
    ...
    bool writeDataToBaseInfo(QString& strBaseInfoItemName,  QMap<QString,QVariant>   mapItemInfo);                     //写入基本信息
    bool updateBaseInfo(QString& strBaseInfoItemName, QMap<QString,QVariant>   mapItemInfo);                    //修改基本信息
    bool writeDataToConfig(QString& strConfigInfoItemName,  QMap<QString,QVariant>   mapItemInfo);                       //写入配置信息
    bool updateConfigInfo(QString& strConfigInfoItemName, QMap<QString,QVariant>   mapItemInfo);                      //修改配置信息
    bool writeDataToEvent(QString& strEventInfoItemName,    QMap<QString,QVariant>   mapItemInfo);                        //写入事件信息
    bool updateEventInfo(QString& strEventInfoItemName,   QMap<QString,QVariant>   mapItemInfo);                       //修改事件信息
    bool removeEvent(QString& strEventInfoItemName,   QMap<QString,QVariant>   mapItemInfo);                       //移除事件信息
    bool writeDataToChannelInfo(short shChannelNo,     QMap<QString,QVariant>   mapItemInfo);                  //写入通道信息
    bool updateChannelInfo(short shChannelNo,    QMap<QString,QVariant>   mapItemInfo);                 //修改通道信息
...
private:
    ...
    //数据库DML操作
    bool    insertIntoTableSingle(QString& strTableName, QVector<QString>& vecValues);                    //将新数据插入到数据库对应表中
    bool    insertIntoTableBatch(QString& strTableName, QVector<QString>& vecValues);                     //批量插入
    ...
    //数据库DQL操作
    void    selectFromTable(QString& strTableName,QString& vecColumns, QString& strCondition, QVector<QVector<QString>>& vecResult);    //从数据库对应表中检索数据



    //数据库备份、还原操作，表的备份、还原操作
    bool backupDataBase(const QString& sourcePath, QString& destinationPath);                                        //数据库备份
    bool restoreRecentVersionDataBase();                                                                                   //回退到最近日期版本的数据库
    bool backupTable(const QString& strTableName, QString& strBackupTableName);                                            //备份表数据
    bool restoreTable(const QString& strTableName, QString& strBackupTableName);                                           //还原表数据
...
private:
    ...
    QVector<QString>   m_vecTableNames;                      //数据库中表的名字集合
